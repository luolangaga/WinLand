...
| `Handler` | `IslandDropContext → Task<string?>`：在 **UI 线程**被调用。**返回的文案会由宿主弹一条临时消息反馈给用户**；返回 `null` / 空字符串时宿主代你弹「已完成「卡片标题」」—— 每个动作都要有反馈 |
...
* **只对文件、文本、图片触发**：其它载荷不会打开投放面板 —— 内容读不出来时面板会**直接收回**，不会停在"正在读取…"上（每次读取都有超时，源进程不回应也不会冻住界面）。
* **类型由宿主按内容判定，是一条退化链**（真文件 → 图片 → 文本）：
  - 「真文件」要求有**真实路径**；浏览器拖网页图片给的是**虚拟文件**（`FileGroupDescriptorW`+`FileContents`，没有路径）—— 宿主会读它的内容，按扩展名认出是图片就转成图片载荷（**SVG 也支持**，原始矢量字节）；
  - 同时有位图和文本时**不会盲目当图片**：浏览器里拖选中的文字常常附带一张「选区快照」位图，那种情况按文本处理；
    只有 HTML 片段里带 `<img>`、或文本本身就是图片地址才算图片；
  - 判定依据与可用格式会写进宿主日志（`plugin.host.log` 的 Debug 行），排错时先看它。
* **宿主内置的五个动作**：打开（默认应用打开）、所在位置（资源管理器里选中）、复制路径、复制文本、保存图片（弹系统「另存为」让用户自己选位置，取消则提示「已取消保存」）。
